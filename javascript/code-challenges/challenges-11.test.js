...
const transformToLis = (obj) =>
  Object.keys(obj).map((item) => `<li>${item}: ${obj[item]}</li>`);
...
------------------------------------------------------------------------------------------------ */
//https://stackoverflow.com/questions/15083103/sum-of-two-dimensional-array
const totalSum = (input) => {
  let sum = 0;
  input.forEach((list) => {
    list.forEach((indexValue) => {
      sum = sum + indexValue;
    });
  });
  return sum;
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((arr) => {
    return arr
      .filter((int) => typeof int === 'number' && int % 5 === 0)
      .map((filterInt) => Math.pow(2, filterInt));
  });
};
...
let findShortest = (data) =>
  data.reduce((a, b) => (+a.height < +b.height ? a : b)).name;
