...
const addValues = (arr) => arr.reduce((acc, cValue) => (acc = acc + cValue), 0);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const addPurchases = (arr) =>
  arr.reduce((acc, cValue) => cValue.purchasePrice + acc, 0);
...
// arr.reduce((acc,cValue) => acc.push(cValue.name), []);
const returnNames = (arr) => {
  const list = [];
  arr.reduce((acc, cValue) => list.push(cValue.name), []);
  return list;
};
...
const countNumberOfChildren = (arr) =>
  arr.reduce(
    (acc, cValue) => (cValue.children ? acc + cValue.children.length : acc),
    0
  );
...
const countPrimeNumbers = (arr) =>
  arr.reduce((acc, cValue) => (isPrime(cValue) ? acc + 1 : acc), 0);
...
const extractStat = (statName, arr) =>
  arr.reduce(
    (acc, cValue) => (cValue.stat.name === statName ? cValue : acc),
    null
  );
...
const extractChildren = (arr) => {
  return arr
    .filter((character) => character.name.indexOf('a') >= 0)
    .reduce((acc, current) => {
      if (current.children) {
