...
const raisedToTheThird = (arr) => {
  let cubeArray = [];
  arr.forEach((element) => { cubeArray.push(Math.pow(element, 3));
  });
  return cubeArray;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i in arr) {
    let ar = arr[i];
    let twoPower = Math.pow(2, ar);
    twoPowerArray.push(twoPower);
  }
  return twoPowerArray;
};
...
const forEachTwoToThe = (arr) => {
  let twoPowerArray = [];
  arr.forEach((element) => {
    twoPowerArray.push(Math.pow(2, element));
  });
  return twoPowerArray;
};
...
const extractStats = (arr) => {
  let statsArray = arr.map((n, idx) => {
    let sum = n.effort + n.baseStat;
    return ({name: n.stat.name,total: sum});
  });
  return statsArray;
};
